...
export async function handler(event){
  try{
    ...
    const [{data:payments},{data:requests},{data:failedJobs}]=await Promise.all([
      ...
      auth.service.from('generation_jobs').select('id,generation_id,last_error,total_count,done_count,created_at,generations(user_id,theme,format)').eq('status','failed').order('created_at',{ascending:false}).limit(20)
    ])
