import { assertSpendCeiling,body,dispatchJob,imageSlug,integer,json,loadCatalog,method,priceOf,rateLimit,requireUser,safeError,text } from './_shared.js'
...
export async function handler(event){
  try{
    ...
    const {data:job,error}=await auth.service.from('generation_jobs').upsert({generation_id:g.id,status:'queued',attempts:0,start_index:position-1,done_count:position-1,total_count:position,image_quality:item.image_quality,credits_each:item.credits,charged_plan:spent.spent_plan||0,charged_extra:spent.spent_extra||0,last_error:null,finished_at:null,updated_at:new Date().toISOString()},{onConflict:'generation_id'}).select('id').single()
